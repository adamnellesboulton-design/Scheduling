...
It treats each line's **requested shift counts as hard** (everyone works exactly
their D10/D5 numbers) and all the **contract rules as hard** (Saturday caps,
one Saturday a month, job-share separation, the opt-in per-line guarantees).
...
- **Per line**: name, `target_d10`, `target_d5`, `stat_days` (paid statutory
...
- `shift_count_feasibility_check` — each line's requested counts are physically
  ...
  Monday-off line whose D10 can't fit the remaining weekdays; a **work-weekly**
  line whose D10 can't cover every business week; a Friday-before-Saturday line
...
- `x[ni, oi] = 1` ⇔ nurse `ni` works date `oi`.

Two structural guarantees are baked in by simply **omitting** variables, so they
can never be violated:

- A nurse is ineligible on an `unavailable_date` ⇒ no variable (**H3**).
- A `fixed_off_mon` line ⇒ no variable on any Monday (the hard "never Mondays").

Statutory holidays get their own binaries `st[ni, oi]` — the solver **chooses**
which holidays each line takes off (paid, not worked), up to entitlement, spread
to protect coverage. `x + st ≤ 1` on a holiday (you can't work a day you're off),
and each line takes exactly its effective entitlement. A taken stat day reduces
that line's **worked** D10 target by one.
...
- If a day is genuinely short (too few eligible lines, or the line totals don't
...
Each line works **exactly** its worked-D10 (target minus taken stat days) and
...
### 4.4 Job share — H8
Lines sharing a `job_share_group` label are two people splitting **one** line, so
**at most one** of them is assigned on any given day.

### 4.5 Opt-in per-line guarantees (hard when ticked)
...
minimizes**. The weights are chosen so the list reads, in practice, as a
priority order (a bigger weight dominates the smaller ones):

| Term | Weight | What it does |
|------|-------:|--------------|
| `W_SHORTFALL` | **8000** | Penalty per **blank** shift. Dominant — fill coverage first. |
| `W_THREE_OF_FOUR` | **1500** | Low-FTE lines (< 0.30) should be active in ≥ 3 of every 4 weeks. |
| `pref` | 60–900 | Honour each line's ticked preferences (off-days, non-consec Sat, clustering). |
| `W_EXTRA` | 600 | Penalty per **over-staffed** slot (avoid needless extras). |
| `cluster_all` | 0 / 600 | Reward everyone's adjacent days off (longer blocks off). |
| `wd_equity` | 30 / 500 | Balance each weekday type fairly across equal-FTE lines and within each line. |
| `sat_spread` | 30 / 350 | Space each line's Saturdays out (penalize back-to-back). |
| `pattern` | 150 / 250 | Keep each line's weekday pattern stable week to week (predictability). |

The exact `pref` / `wd_equity` / `sat_spread` / `cluster_all` / `pattern` values
come from the chosen **profile** (next section). Hard constraints are *not* in
this sum — they're enforced separately and always hold.
...
- **Cluster-maximizing** — `cluster_all` (600) is high; groups each line's shifts
...
2. produces a **best-effort greedy** schedule (`_greedy`) that fills each day
   with the eligible lines furthest below their target hours, while honouring the
   Saturday cap, job-share separation and the per-line guarantees (including the
